...
The `auto_explain` extension can be applied to the entire session by adding it to the `shared_preload_libraries` parameter in the parameter group. For the remaining extensions, if you need SUPERUSER permission, you can control it by referring to the [Extension Management](db-instance-ngovc/#extension) item or install the required extensions with the [CREATE EXTENSION](https://www.postgresql.org/docs/17/sql-createextension.html) syntax.
