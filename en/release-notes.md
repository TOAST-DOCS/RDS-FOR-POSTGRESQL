...
- Fixed an issue where the response format of the DB engine version query API was returned differently from the format specified in the API guide, and updated it to match the guide.
- Fixed an issue where required value handling specified in the guide did not work correctly when requesting point-in-time restore or backup restore via API.
