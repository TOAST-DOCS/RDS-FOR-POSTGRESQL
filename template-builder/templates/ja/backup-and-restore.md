...
従来のバックアップ方式は、バックアップアプリケーションがDBインスタンスで実行されるため、バックアップ中に性能低下が発生する可能性がありましたが、**ストレージスナップショットバックアップ**はPrimaryまたは`archive_mode`パラメータがalwaysのDBインスタンスに限り、Cinderストレージスナップショットを活用してバックアップを実行します。
...
* 共用バックアップサーバー料金: バックアップデータの検証および変換のために使用する共用バックアップサーバーの利用料金です。
