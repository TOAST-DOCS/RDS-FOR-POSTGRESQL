...
You can proceed to upgrade the major version through Primary DB instance modifications. The order of execution is as follows.
...
- Proceed by selecting one of the DB instances (including Standby) in non-Primary replication relationship.
    - Proceed the version upgrade for selected DB instances.
        ...
        - [Note] At this stage, **write loads** to the Primary are blocked and only read loads can be processed.
...
In RDS for PostgreSQL, minor version upgrades can be performed on Read Replicas as well as Primaries, and if performed, the version upgrade will be performed on the target DB instance. (For high availability Primaries, version upgrade will also proceed for the Standby.)
