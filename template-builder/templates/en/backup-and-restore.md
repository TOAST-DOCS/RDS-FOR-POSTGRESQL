...
![db-instance-detail-backup]({{url.cdn}}/20260609/db-instance-detail-backup-{{lang}}.png)
...
![backup-create]({{url.cdn}}/20241210/backup-create-{{lang}}.png)
...
![db-instance-list-export-obs]({{url.cdn}}/20260609/db-instance-list-export-obs-{{lang}}.png)

![db-instance-list-export-obs-modal]({{url.cdn}}/20260609/db-instance-list-export-obs-modal-{{lang}}.png)
...
![db-instance-detail-backup-export]({{url.cdn}}/20260609/db-instance-detail-backup-export-{{lang}}.png)
...
![backup-export]({{url.cdn}}/20241210/backup-export-{{lang}}.png)
...
With the existing backup method, the backup application runs on the DB instance, so performance degradation can occur during a backup. **Storage snapshot backup**, on the other hand, uses Cinder storage snapshots to perform backups, and is available only for a Primary or a DB instance whose `archive_mode` parameter is `always`.
...
* Shared backup server fee: The usage fee for the shared backup server used to validate and convert backup data.
    * Even though shared resources are used, each customer is charged only for the time they actually used.
...
![backup-restore]({{url.cdn}}/20241210/backup-restore-{{lang}}.png)
...
![db-instance-pitr]({{url.cdn}}/20260609/db-instance-pitr-{{lang}}.png)
