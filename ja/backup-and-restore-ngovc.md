...
![backup-config](https://static-station.ngovc.com/v1/AUTH_3365819a41194e7ca358853f5b2eec52/cdn/prod_rds_postgres/20241210/backup-config-ja.png)
...
![db-instance-detail-backup](https://static-station.ngovc.com/v1/AUTH_3365819a41194e7ca358853f5b2eec52/cdn/prod_rds_postgres/20260609/db-instance-detail-backup-ja.png)
...
![backup-create](https://static-station.ngovc.com/v1/AUTH_3365819a41194e7ca358853f5b2eec52/cdn/prod_rds_postgres/20241210/backup-create-ja.png)
...
![db-instance-list-export-obs](https://static-station.ngovc.com/v1/AUTH_3365819a41194e7ca358853f5b2eec52/cdn/prod_rds_postgres/20260609/db-instance-list-export-obs-ja.png)

![db-instance-list-export-obs-modal](https://static-station.ngovc.com/v1/AUTH_3365819a41194e7ca358853f5b2eec52/cdn/prod_rds_postgres/20260609/db-instance-list-export-obs-modal-ja.png)
...
![db-instance-detail-backup-export](https://static-station.ngovc.com/v1/AUTH_3365819a41194e7ca358853f5b2eec52/cdn/prod_rds_postgres/20260609/db-instance-detail-backup-export-ja.png)

❶バックアップを実行した原本のDBインスタンスの詳細タブでエクスポートするバックアップファイルを選択し、**オブジェクトストレージにバックアップをエクスポート**をクリックすると、バックアップをエクスポートするためのポップアップ画面が表示されます。

![backup-export](https://static-station.ngovc.com/v1/AUTH_3365819a41194e7ca358853f5b2eec52/cdn/prod_rds_postgres/20241210/backup-export-ja.png)
...
既存のバックアップ方式は、バックアップアプリケーションがDBインスタンスで実行されるため、バックアップ中にパフォーマンスの低下が発生する可能性がありますが、**ストレージスナップショットバックアップ**は、マスターまたは `archive_mode` パラメータが `always` のDBインスタンスに限り、Cinderストレージスナップショットを活用してバックアップを実行します。
スナップショット作成後、パブリックバックアップサーバーでデータの有効性検証及びファイル変換を実行するため、バックアップ中もDBインスタンスのパフォーマンスに影響を与えません。

**主な特徴**
* パフォーマンスの維持: バックアップ実行中もDBインスタンスのパフォーマンスが100%維持されます。
* 安定性の向上: 別の検証プロセスを通じてバックアップデータの信頼性を保証します。
* 高可用性の一時停止: スナップショット作成時点にはデータ整合性のために高可用性機能が一時停止される場合があります。
...
![db-instance-detail-backup-restore](https://static-station.ngovc.com/v1/AUTH_3365819a41194e7ca358853f5b2eec52/cdn/prod_rds_postgres/20260609/db-instance-detail-backup-restore-ja.png)

❶DBインスタンスの詳細タブで復元するバックアップファイルを選択した後、**バックアップ復元**をクリックすると、DBインスタンスの復元画面に移動します。

または

![backup-restore](https://static-station.ngovc.com/v1/AUTH_3365819a41194e7ca358853f5b2eec52/cdn/prod_rds_postgres/20241210/backup-restore-ja.png)
...
![db-instance-pitr](https://static-station.ngovc.com/v1/AUTH_3365819a41194e7ca358853f5b2eec52/cdn/prod_rds_postgres/20260609/db-instance-pitr-ja.png)
...
![db-instance-pitr-timestamp](https://static-station.ngovc.com/v1/AUTH_3365819a41194e7ca358853f5b2eec52/cdn/prod_rds_postgres/20260609/db-instance-pitr-timestamp-ja.png)
...
(1) [バックアップファイルエクスポート](backup-and-restore-ngovc/#_5)項目を参考にして、RDS for PostgreSQLのバックアップをオブジェクトストレージにエクスポートします。

(2)復元するプロジェクトのコンソールに接続して、**DBインスタンス**タブで**オブジェクトストレージにあるバックアップで復元**ボタンをクリックします。

![backup-obs-restore](https://static-station.ngovc.com/v1/AUTH_3365819a41194e7ca358853f5b2eec52/cdn/prod_rds_postgres/20241210/backup-obs-restore-ja.png)
...
❺コンテナに保存されるバックアップのパスを入力します。フォルダ名は最大255バイト、フルパスは最大1024バイトです。特定の形式(.または..)は使用できず、特殊文字(' " < > ;)や空白は入力できません。
❻ [DBインスタンス作成](db-instance-ngovc/#db_1)項目を参照して残りの設定を入力し、**オブジェクトストレージにあるバックアップで復元**ボタンをクリックします。
