...
![db-instance-detail-db-sync](https://static.toastoven.net/prod_rds_postgres/20260210/db-instance-detail-db-sync-ja.png)
...
![db-instance-detail-user-sync](https://static.toastoven.net/prod_rds_postgres/20260210/db-instance-detail-user-sync-ja.png)
...
![db-instance-detail-hba-create](https://static.toastoven.net/prod_rds_postgres/20260414/db-instance-detail-hba-create-ja.png)
...
![db-instance-detail-hba-create-by-text](https://static.toastoven.net/prod_rds_postgres/20260414/db-instance-detail-hba-create-by-text-ja.png)
...
RDS for PostgreSQLでは、データベースとユーザーを簡単に管理できるようにコンソールで管理機能を提供していますが、ユーザーが独自に制御できるように設定する機能も提供しています。直接制御を使用すると、現在作成されている全てのDDLユーザーに`CREATEDB`、`CREATEROLE`権限を付与します。既存のユーザーの権限をDDLで変更したり、新規作成したりする場合にも同様に付与します。
...
PostgreSQLを再起動するにはDBインスタンスを再起動します。再起動時間を最小化するため、サービス負荷が低い時間帯に実行することを推奨します。
