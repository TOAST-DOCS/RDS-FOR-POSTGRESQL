...
> バックアップが実行されるDBインスタンスの場合、Read Replica作成の過程でストレージI/Oのパフォーマンスが低下する場合があります。

> [注記]
> リードレプリカの作成プロセスに必要なデータストレージのサイズ分、バックアップストレージの料金が発生する場合があります。
...
> [注意]
> リージョンピアリングが接続されていても、ルート設定が正しくない場合、Read Replica の作成に失敗したり、レプリケーションが中断される場合があります。
...
> [注意]
> Primary DB インスタンスのステータスが異常な場合は、昇格操作を実行することはできません。
...
> [注記]
> 高可用性DBインスタンスは、PostgreSQLのクエリ文で他のDBインスタンスまたは外部PostgreSQLのPrimaryから強制的にレプリケートするように設定した場合、高可用性および一部の機能が正常に動作しません。
...
> [注記]
> Primaryのデータストレージ使用量がいっぱいになると、高可用性監視プロセスが障害として検知してフェイルオーバーを実行するため、注意してください。
...
> [注記]
> 高可用性機能はドメインを基盤としているため、接続を試みるクライアントがDNSサーバーに接続できないネットワーク環境の場合、ドメインでDBインスタンスに接続できず、フェイルオーバー発生時に正常な接続が不可能になります。
...
> バックアップが実行されるDBインスタンスの場合、Failed Over Primaryの再構築の過程でストレージI/Oのパフォーマンスが低下する場合があります。
...
> [注意]
> StandbyおよびレプリケーショングループのRead Replicaのレプリケーション遅延値が1以上の場合、レプリケーション遅延が発生したとみなされ、このとき手動フェイルオーバーは失敗します。負荷が少ない時間帯に手動フェイルオーバーを実施することをお勧めします。レプリケーション遅延による再起動の失敗はイベント画面で確認できます。
