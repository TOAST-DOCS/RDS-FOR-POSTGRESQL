...
- DBエンジンバージョン照会 API レスポンス形式の不具合を修正
    - DBエンジンバージョン照会 API のレスポンス形式が API ガイドに明示された形式と異なる形で返される問題を修正し、ガイドと一致するように変更しました。
- 復元 API 必須値の処理不具合を修正
...
- 高可用性DBインスタンスのアクセス制御ルール不一致の問題を修正
    - 特定の状況で以前に適用したアクセス制御ルールがマスターと予備マスター間で不一致になっていた問題を修正しました。
- 削除不可ユーザーが削除済みとして表示される問題を修正
    - オブジェクトを所有しているため削除できないユーザーが、コンソールでは削除済みとして誤って表示される問題を修正しました。
    - オブジェクトを所有するユーザーを削除する際に追加オプションを選択して正常に削除できるように改善しました。
