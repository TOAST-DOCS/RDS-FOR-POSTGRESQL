...
- DB エンジンバージョン照会 API レスポンス形状エラーの修正
    - DB エンジンバージョン照会 API のレスポンス形状が API ガイドに明示された形状と異なる形で返される問題を修正し、ガイドと一致するように変更しました。
- 復元 API 必須値処理エラーの修正
    - API による時点復元またはバックアップ復元リクエスト時に、ガイドに明示された必須値の処理が正常に動作しない問題を修正しました。
