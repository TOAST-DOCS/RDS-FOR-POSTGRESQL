<!-- pre-align:aligned sig=ad5705aa30ae -->
...
<a id="change-the-maxconnections-parameter"></a>
### `max_connections` 파라미터 변경 { #change-the-maxconnections-parameter }

읽기 복제본이 추가된 상태에서 `max_connections` 값을 변경할 때는 적용 순서에 따라 발생하는 문제가 있습니다. 마스터의 값이 읽기 복제본보다 작은 값으로 설정돼야 하므로 마스터의 값만 변경하는 경우 읽기 복제본보다 큰 값으로 적용할 수 없습니다. 마찬가지 이유로 읽기 복제본의 값만 변경하는 경우 마스터보다 작은 값으로 적용할 수 없습니다. 필요한 경우 DB 인스턴스 그룹 단위로 전체 적용하는 것을 권장합니다.

<a id="change-the-sharedbuffers-parameter"></a>
### `shared_buffers` 파라미터 변경 { #change-the-sharedbuffers-parameter }
