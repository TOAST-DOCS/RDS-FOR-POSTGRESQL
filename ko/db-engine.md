...
- Primary DB 인스턴스에서 버전 업그레이드 사전 점검을 수행합니다.
    - 사전 점검 결과 문제가 없으면 버전을 업그레이드합니다.
    - 사전 점검 결과는 로그 파일 형태로 제공되며 DB 인스턴스 상세의 **로그** 탭에서 `pg_upgrade.log` 파일로 확인할 수 있습니다.
- DB 인스턴스 그룹 내 Primary가 단독으로 존재하면, Primary DB 인스턴스의 버전을 업그레이드합니다.
...
- Primary 외 복제 관계의 DB 인스턴스(Standby 포함) 중 DB 인스턴스 하나를 선정해 진행합니다.
    - 선정한 DB 인스턴스의 버전을 업그레이드합니다.
        - Standby가 있으면 Read Replica에 우선하여 버전을 업그레이드합니다.
...
RDS for PostgreSQL에서 마이너 버전 업그레이드는 Primary뿐만 아니라 Read Replica에서도 실행할 수 있으며, 실행하면 실행한 대상 DB 인스턴스의 버전이 업그레이드됩니다. 고가용성 구성이 완료된 Primary는 Standby도 함께 업그레이드합니다.

<a id="perform-version-upgrades-minor-version-upgrade-order"></a>
#### 마이너 버전 업그레이드 순서

- Primary의 버전 업그레이드를 시도할 때 Standby가 존재하면 함께 버전을 업그레이드합니다.
    - Primary 단독으로 버전 업그레이드가 진행되면 다운타임이 발생합니다.
    - Standby와 함께 버전 업그레이드가 진행되면 과정 중 장애 조치를 이용한 재시작이 동반되며, 순단이 발생할 수 있습니다.
- Read Replica의 버전을 업그레이드할 때는 해당 DB 인스턴스 단독으로 버전을 업그레이드합니다.
    - 버전 업그레이드 중에는 다운타임이 발생합니다.
    - 실패하면 복구 작업이 시도될 수 있으며, 복구에 성공하면 버전 업그레이드 전 상태로 되돌아갑니다.
    - 복구 작업에도 실패하면 재구축으로 복구를 시도할 수 있습니다.
