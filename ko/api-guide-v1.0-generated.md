...
| 리전 | 엔드포인트 |
|------|-----------|
| 한국(판교) | https://kr1-rds-postgres.api.nhncloudservice.com |
...
| 권한 | 설명 |
|------|------|
| RDS for PostgreSQL ADMIN | API 실행에 필요한 모든 권한이 부여됩니다. |
| RDS for PostgreSQL VIEWER | 정보를 조회하는 권한만 부여됩니다. |
...
| backup.backupSchedules.backupWndBgnTime | Body | String | X | 백업 시작 시간<br/>- 예시: `00:00:00` |
| backup.backupSchedules.backupWndDuration | Body | Enum | X | 백업 윈도우<br/>백업 시작 시간부터 설정된 기간 안에 자동 백업이 실행됩니다.<br/>- `HALF_AN_HOUR`: 30분<br/>- `ONE_HOUR`: 1시간<br/>- `ONE_HOUR_AND_HALF`: 1시간 30분<br/>- `TWO_HOURS`: 2시간<br/>- `TWO_HOURS_AND_HALF`: 2시간 30분<br/>- `THREE_HOURS`: 3시간 |
<details><summary>예시</summary>

```json
{
    "dbInstanceName": "DB 인스턴스를 식별할 수 있는 이름",
    "description": "DB 인스턴스에 대한 추가 정보",
    "dbFlavorId": "00000000-0000-0000-0000-000000000001",
    "dbVersion": "DB 버전 정보",
    "dbPort": 0,
    "databaseName": "DB 엔진 내 신규 생성할 데이터베이",
    "dbUserName": "DB 엔진 내 신규 생성할 사용자 계",
    "dbPassword": "DB 엔진 내 신규 생성할 사용자 계",
    "parameterGroupId": "00000000-0000-0000-0000-000000000001",
    "dbSecurityGroupIds": [],
    "useDeletionProtection": false,
    "useDefaultNotification": false,
    "useHighAvailability": false,
    "pingInterval": 0,
    "failoverReplWaitingTime": 0,
    "network": {
        "subnetId": "00000000-0000-0000-0000-000000000001",
        "usePublicAccess": false,
        "availabilityZone": "DB 인스턴스를 생성할 가용성 영역<"
    },
    "storage": {
        "storageType": "데이터 스토리지 유형<br/>- 예시",
        "storageSize": 0
    },
    "backup": {
        "backupPeriod": 0,
        ...
        "backupSchedules": [
            {
                "backupWndBgnTime": "백업 시작 시간<br/>- 예시: `",
                "backupWndDuration": "백업 윈도우<br/>백업 시작 시간부"
            }
        ]
    }
}
...
```json
{
    "tenantId": "백업이 저장될 오브젝트 스토리지의 테",
    "username": "NHN Cloud 계정 또는 IAM ",
    "password": "백업이 저장될 오브젝트 스토리지의 A",
    "targetContainer": "백업이 저장될 오브젝트 스토리지의 컨",
    "objectPath": "컨테이너에 저장될 백업의 경로"
}
```
</details>
...
```json
{
    ...
    "maintWndBgnTime": "자동 유지보수 시작 시간 <br/>-",
    "maintWndDuration": "유지보수 윈도우 <br/> 예시: `",
...
```json
{
    "dbUserName": "DB 사용자 계정 이름<br/>- 최",
    "dbPassword": "DB 사용자 계정 암호<br/>- 최",
    "authorityType": "DB 사용자 권한 유형<br/>- `",
    "createDefaultHbaRules": false,
    "address": "기본 접근 제어 규칙 생성 시 사용할"
}
```
</details>
...
```json
{
    "databaseApplyType": "데이터베이스 규칙 적용 방식<br/>",
    "dbUserApplyType": "DB 사용자 규칙 적용 방식<br/>",
    "databaseIds": [],
    "dbUserIds": [],
    "address": "접속 주소<br/>- CIDR 형식,",
    "authMethod": "인증 방식<br/>- `TRUST`:"
}
```
</details>
...
| backup.backupSchedules | Body | Array | X | 백업 스케줄 목록 |
| backup.backupSchedules.backupWndBgnTime | Body | String | X | 백업 시작 시간<br/>- 예시: `00:00:00` |
| backup.backupSchedules.backupWndDuration | Body | Enum | X | 백업 Duration<br/>백업 시작 시간부터 설정된 기간 안에 자동 백업이 실행됩니다.<br/>- `HALF_AN_HOUR`: 30분<br/>- `ONE_HOUR`: 1시간<br/>- `ONE_HOUR_AND_HALF`: 1시간 30분<br/>- `TWO_HOURS`: 2시간<br/>- `TWO_HOURS_AND_HALF`: 2시간 30분<br/>- `THREE_HOURS`: 3시간 |
<details><summary>예시</summary>
...
```json
{
    ...
    "backup": {
        ...
        "backupSchedules": [
            {
                "backupWndBgnTime": "백업 시작 시간<br/>- 예시: `",
                "backupWndDuration": "백업 Duration<br/>백업 시"
            }
        ]
    }
}
...
```json
{
    "dbSecurityGroupName": "DB 보안 그룹을 식별할 수 있는 이",
    "description": "DB 보안 그룹에 대한 추가 정보",
    "rules": [
        {
            "description": "DB 보안 그룹 규칙에 대한 추가 정",
            "direction": "통신 방향<br/>- `INGRESS",
            "etherType": "Ether 유형<br/>- `IPV4",
            "cidr": "허용할 트래픽의 원격 소스<br/>-",
            "port": {
                "portType": "포트 유형<br/>- `DB_PORT",
                "minPort": 0,
                "maxPort": 0
            }
        }
...
```json
{
    "parameterGroupName": "파라미터 그룹을 식별할 수 있는 이름",
    "description": "파라미터 그룹에 대한 추가 정보",
    "dbVersion": "DB 버전 정보"
}
```
</details>
...
```json
{
    "modifiedParameters": [
        {
            "parameterName": "00000000-0000-0000-0000-000000000001",
            "value": "변경할 파라미터 값"
        }
    ]
}
...
```json
{
    "userGroupName": "사용자 그룹을 식별할 수 있는 이름",
    "memberIds": [],
    "selectAllYN": false
}
```
...
```json
{
    "notificationGroupName": "알림 그룹을 식별할 수 있는 이름",
    "notifyEmail": false,
    "notifySms": false,
    "isEnabled": false,
    "dbInstanceIds": [],
...
| 이름 | 종류 | 형식 | 필수 | 설명 |
|------|------|------|------|------|
| notificationGroupId | URL | String | O |  |
#### 응답
...
| threshold | Body | Number | O | 감시 대상 임곗값 |
| duration | Body | Number | O | 감시 대상 지속 시간<br/>- 단위: `분` |
<details><summary>예시</summary>

```json
{
    "metricName": "감시 대상 성능 지표<br/>- 설정",
    "comparisonOperator": "감시 대상 비교 방법<br/>- `L",
    "threshold": 0,
    "duration": 0
}
```
...
| metricStatistics.values.timestamp | Body | String | 측정 시간 |
